            ...
            {athletes.slice(0, 6).map(a => (
              <button
                ...
                className={`font-condensed text-[10px] tracking-widest px-2.5 py-1 border transition-colors ${
                  a.checkedIn
                    ? 'border-[#22c55e]/30 text-[#22c55e]/50 bg-[#22c55e]/5 cursor-default'
                ...
                {log.length === 0 ? (
                  ...
                  log.map((entry, i) => (
                    <div key={i} className={`px-4 py-3.5 flex items-center gap-3 ${
                      ...
                      entry.status === 'already_checked' ? 'bg-[#f59e0b]/2' : 'bg-[#C8102E]/2'
                    }`}>
                      {entry.status === 'success'
            ...
            <div>
              ...
              <div className="divide-y divide-white/4 max-h-[480px] overflow-y-auto">
                {filteredAthletes.map(a => (
                  <div key={a.id} className="px-4 py-3.5 flex items-center gap-3 hover:bg-white/1 transition-colors">
                    <div className={`w-9 h-9 flex items-center justify-center flex-shrink-0 font-bebas text-sm transition-colors ${a.checkedIn ? 'bg-[#22c55e]/20 text-[#22c55e]' : 'bg-white/5 text-white/30'}`}>
                      {a.checkedIn ? '✓' : a.bib}
                    </div>
                    <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className={`font-condensed text-[9px] tracking-[2px] uppercase px-2 py-0.5 ${a.checkedIn ? 'bg-[#22c55e]/15 text-[#22c55e]' : 'bg-white/5 text-white/25'}`}>
