          ...
          <div className="flex gap-1.5 overflow-x-auto pb-1">
            {categories.map(c => (
              <button key={c} onClick={() => setCatFilter(c)}
                className={`font-condensed text-xs tracking-[2px] uppercase px-3 py-2 transition-colors flex-shrink-0 ${catFilter === c ? 'bg-[#C9A84C]/20 border border-[#C9A84C]/50 text-[#C9A84C]' : 'bg-[#111] border border-white/8 text-white/40 hover:text-white'}`}>
